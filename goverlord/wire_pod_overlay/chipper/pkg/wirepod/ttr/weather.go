...
// geocodeOpenWeatherMap tries a spoken location phrase against OpenWeatherMap's
// geocoding endpoint, which only matches strict "city[,state][,country]"
// comma formatting - a raw speech-parsed phrase like "windsor ontario canada"
// (meant as city+state+country) doesn't land in that shape on the first try.
// Tries candidates in order (see geocodeCandidates) and returns the first
// non-empty match, so callers don't need to know which grouping worked.
...
func getWeather(location string, botUnits string, hoursFromNow int) (string, string, string, string, string, string) {
	...
	if weatherEnabled {
		if weatherAPIProvider == "weatherapi.com" {
			...
			if err != nil {
				logger.Println("weather error:", err)
				return "undefined", "false", "test", location, "120", "C"
			}
			...
			geoCodingInfoStruct := geocodeOpenWeatherMap(location, weatherAPIKey)
			...
			if hoursFromNow == 0 {
				weatherURL = "https://api.openweathermap.org/data/2.5/weather?lat=" + Lat + "&lon=" + Lon + "&units=" + units + "&appid=" + weatherAPIKey
			} else {
				weatherURL = "https://api.openweathermap.org/data/2.5/forecast?lat=" + Lat + "&lon=" + Lon + "&units=" + units + "&appid=" + weatherAPIKey
			}
